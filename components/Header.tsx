  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-ink/88 text-white backdrop-blur-xl">
      <div className="container-shell flex h-16 items-center justify-between gap-6">
        <Link href="/" className="flex items-center gap-3" aria-label={`${company.nameJa} トップへ`}>
        ...
        <nav className="hidden items-center gap-6 text-sm text-white/74 lg:flex" aria-label="メインナビゲーション">
          {navItems.map((item) => (
